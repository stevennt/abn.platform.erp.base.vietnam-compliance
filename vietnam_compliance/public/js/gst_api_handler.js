...
taxpayer_api.call = async function (...args) {
    ...
    await vietnam_compliance.authenticate_otp(message.gstin, message.error_type);
...
Object.assign(vietnam_compliance, {
    get_gstin_otp(company_gstin, error_type) {
        ...
        return new Promise((resolve) => {
            const prompt = new frappe.ui.Dialog({
                ...
                secondary_action() {
                    frappe.call({
                        method: "vietnam_compliance.vat_vietnam.utils.gstr_utils.request_otp",
                        args: { company_gstin },
                        callback: function () {
                            frappe.show_alert({
    ...
    async authenticate_otp(gstin, error_type = null) {
        ...
        while (!is_authenticated) {
            ...
            const { message } = await frappe.call({
                method: "vietnam_compliance.vat_vietnam.utils.gstr_utils.authenticate_otp",
    ...
    generate_evc_otp(company_gstin, pan, request_type) {
        return taxpayer_api.call({
            method: "vietnam_compliance.vat_vietnam.utils.gstr_utils.generate_evc_otp",
            args: {
