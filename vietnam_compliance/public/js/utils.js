...
Object.assign(vietnam_compliance, {
    MONTH: [
        "January",
        "February",
        "March",
    ...
    check_duplicate_party(field, value, party_type, party = null) {
        ...
        frappe.call({
            method: "vietnam_compliance.vat_vietnam.utils.check_duplicate_party",
    ...
    get_gstin_query(party, party_type = "Company", exclude_isd = false) {
        ...
        return {
            query: "vietnam_compliance.vat_vietnam.utils.get_gstin_list",
            params: { party, party_type, exclude_isd },
        };
    },

    async get_gstin_options(party, party_type = "Company") {
        const { query, params } = vietnam_compliance.get_gstin_query(party, party_type);
    ...
    async get_account_options(company) {
        if (!company) return;
        const { message } = await frappe.call({
            method: "vietnam_compliance.vat_vietnam.utils.get_all_gst_accounts",
            args: {
                company,
            },
        });
    ...
    async set_gstin_status(field, doc, force_update = false) {
        ...
        let { message } = await frappe.call({
            method: "vietnam_compliance.vat_vietnam.doctype.gstin.gstin.get_gstin_status",
        ...
        field.set_description(
            vietnam_compliance.get_gstin_status_desc(message?.status, message?.last_updated_on),
        );
    ...
    async set_pan_status(field, force_update = false) {
        ...
        let { message } = await frappe.call({
            method: "vietnam_compliance.vat_vietnam.doctype.pan.pan.get_pan_status",
        ...
        refresh_btn.on("click", async function () {
            await vietnam_compliance.set_pan_status(field, true);
        });
    ...
    validate_gst_transporter_id(transporter_id, doc) {
        ...
        frappe.call({
            method: "vietnam_compliance.vat_vietnam.doctype.gstin.gstin.validate_gst_transporter_id",
    ...
    set_gstin_refresh_btn(field, doc) {
        ...
        refresh_btn.on("click", async function () {
            await vietnam_compliance.set_gstin_status(field, doc, true);
        });
    },
    ...
    is_api_enabled(settings) {
        ...
        return settings.enable_api && vietnam_compliance.can_enable_api(settings);
    },

    is_e_invoice_enabled() {
        return vietnam_compliance.is_api_enabled() && gst_settings.enable_e_invoice;
    },
    ...
    setup_itc_claim_period_query(frm) {
        frm.set_query("itc_claim_period", () => ({
            query: "vietnam_compliance.vat_vietnam.utils.itc_claim.get_itc_period_options",
            params: {
...
// DO NOT USE IN CODE
window.ic = window.vietnam_compliance;
