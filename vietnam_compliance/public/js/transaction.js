...
async function update_gst_details(frm, event) {
    ...
    const party_type = vietnam_compliance.get_party_type(frm.doc.doctype).toLowerCase();
    ...
    vietnam_compliance.fetch_and_update_gst_details(frm, args);
}

vietnam_compliance.fetch_and_update_gst_details = function (frm, args, method) {
    frappe.call({
        method: method || "vietnam_compliance.vat_vietnam.overrides.transaction.get_gst_details",
        args,
        async callback(r) {
            if (!r.message) return;
    ...
    frappe.ui.form.on(doctype, {
        gst_category(frm) {
            ...
            if (
                ...
                !vietnam_compliance.is_indian_registered_company(frm.doc.company)
            )
                return;
...
function set_and_validate_gstin_status(doctype) {
    ...
    frappe.ui.form.on(doctype, {
        ...
        gst_transporter_id(frm) {
            vietnam_compliance.validate_gst_transporter_id(frm.doc.gst_transporter_id, frm.doc);
        },

        posting_date(frm) {
            if (frm.get_field("posting_date")) _set_and_validate_gstin_status(frm, gstin_field_name);
        },

        transaction_date(frm) {
            if (frm.get_field("transaction_date")) _set_and_validate_gstin_status(frm, gstin_field_name);
        },
    });
}
...
async function _set_gstin_status(frm, gstin_field_name) {
    ...
    if (!gstin_doc) {
        gstin_doc = await vietnam_compliance.set_gstin_status(gstin_field, frm.doc);

        frm._gstin_doc = frm._gstin_doc || {};
        frm._gstin_doc[gstin] = gstin_doc;
    } else {
        gstin_field.set_description(
            vietnam_compliance.get_gstin_status_desc(gstin_doc?.status, gstin_doc?.last_updated_on),
        );
    }
...
function validate_gstin_status(gstin_doc, frm, gstin_field_name) {
    ...
    if (!registration_date || transaction_date < registration_date)
        frappe.throw({
            message: __("{0} is Registered on {1}. Please make sure that the {2} is on or after {1}", [
    ...
    if (gstin_doc.status === "Cancelled" && transaction_date >= cancelled_date)
        frappe.throw({
            message: __("{0} is Cancelled from {1}. Please make sure that the {2} is before {1}", [
                gstin_field.df.label,
...
function show_gst_invoice_no_banner(frm) {
    ...
    if (!is_invoice_no_validation_required(frm.doc.transaction_type || frm.doc.document_type)) return;
...
function is_invoice_no_validation_required(transaction_type) {
    return (
        ...
        (transaction_type === "Purchase Invoice" && gst_settings.enable_e_waybill_from_pi) ||
        ...
        supplier(frm) {
            ...
            setTimeout(() => {
                ...
                erpnext.utils.get_party_details(
                    frm,
                    "vietnam_compliance.vat_vietnam.overrides.transaction.get_party_details_for_subcontracting",
                    args,
                    () => {
