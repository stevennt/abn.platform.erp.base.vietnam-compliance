frappe.ui.form.on("Sales Invoice", {
    refresh(frm) {
        ...
        if (!vietnam_compliance.is_e_invoice_enabled() || !is_valid_e_invoice_applicability_date(frm)) return;
        ...
        if (!frm.doc.irn && frappe.perm.has_perm(frm.doctype, 0, "submit", frm.doc.name)) {
            frm.add_custom_button(
                ...
                () => {
                    frappe.call({
                        method: "vietnam_compliance.vat_vietnam.utils.e_invoice.generate_e_invoice",
                        args: { docname: frm.doc.name, force: true },
                        callback: async (r) => {
                            if (r.message?.error_code == "2283") {
                                await taxpayer_api.call({
                                    method: "vietnam_compliance.vat_vietnam.utils.e_invoice.handle_duplicate_irn_error",
                                    args: r.message,
                                });
                            }
                            frm.refresh();
        ...
        if (
            ...
            vietnam_compliance.make_text_red("e-Invoice", "Cancel");
        }
    },
    async on_submit(frm) {
        ...
        await frappe.xcall("vietnam_compliance.vat_vietnam.utils.e_invoice.generate_e_invoice", {
    ...
    before_cancel(frm) {
        ...
        return new Promise((resolve) => {
            ...
            if (!is_irn_cancellable(frm) || !vietnam_compliance.is_e_invoice_enabled()) {
                let message = "";
...
function show_cancel_e_invoice_dialog(frm, callback) {
    ...
    vietnam_compliance.primary_to_danger_btn(d);
    d.show();
...
function show_mark_e_invoice_as_generated_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "vietnam_compliance.vat_vietnam.utils.e_invoice.mark_e_invoice_as_generated",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_mark_e_invoice_as_cancelled_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "vietnam_compliance.vat_vietnam.utils.e_invoice.mark_e_invoice_as_cancelled",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function is_e_invoice_generatable(frm, show_message = false) {
    ...
    let is_invalid_invoice_number = vietnam_compliance.validate_invoice_number(frm.doc.name);
...
function is_e_invoice_applicable(frm, show_message = false) {
    if (
        !vietnam_compliance.is_e_invoice_enabled() ||
