...
frappe.listview_settings[DOCTYPE].onload = function (list_view) {
    ...
    if (vietnam_compliance.is_e_invoice_enabled())
        add_bulk_action_for_invoices(
            list_view,
...
async function generate_e_waybill_json(docnames) {
    const ewb_data = await frappe.xcall(
        "vietnam_compliance.vat_vietnam.utils.e_waybill.generate_e_waybill_json",
        { doctype: DOCTYPE, docnames },
    );

    vietnam_compliance.trigger_file_download(ewb_data, get_e_waybill_file_name());
}
...
function show_bulk_update_transporter_dialog(docnames) {
    const d = get_generate_e_waybill_dialog({
        ...
        primary_action(values) {
            ...
            frappe.call({
                method: "vietnam_compliance.vat_vietnam.utils.e_waybill.bulk_update_transporter_in_docs",
                args: {
                    doctype: DOCTYPE,
                    docnames,
                    values,
...
async function bulk_e_waybill_print(docnames) {
    frappe.call({
        method: "vietnam_compliance.vat_vietnam.utils.e_waybill.get_valid_and_invalid_e_waybill_log",
        args: {
            doctype: DOCTYPE,
...
async function enqueue_bulk_e_waybill_generation(docnames) {
    enqueue_bulk_generation("vietnam_compliance.vat_vietnam.utils.e_waybill.enqueue_bulk_e_waybill_generation", {
        doctype: DOCTYPE,
        docnames,
    });
}
...
async function enqueue_bulk_e_invoice_generation(docnames) {
    enqueue_bulk_generation("vietnam_compliance.vat_vietnam.utils.e_invoice.enqueue_bulk_e_invoice_generation", {
        docnames,
    });
}
