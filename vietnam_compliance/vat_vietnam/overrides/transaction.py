...
from vietnam_compliance.vat_vietnam.constants.custom_fields import E_WAYBILL_INV_FIELDS
from vietnam_compliance.vat_vietnam.doctype.gst_settings.gst_settings import (
    restrict_gstr_1_transaction_for,
)
from vietnam_compliance.vat_vietnam.doctype.gstin.gstin import get_and_validate_gstin_status
...
def get_tds_amount(doc):
    tds_accounts = []  # TODO: Vietnam TDS equivalent
    tds_amount = 0
    for row in doc.taxes:
...
def get_tax_template(master_doctype, company, is_inter_state, state_code, is_reverse_charge):
    tax_categories = frappe.get_all(
        ...
        or_filters={"is_vietnam_compliance_default": 1, "gst_state": ["!=", ""]},
    )
